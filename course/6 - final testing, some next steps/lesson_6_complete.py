...
def main(*args):
    ...
    # add to set
    set_xml = generate_set_xml(set_id=set_id, mms_id=mms_id, 
    ...
    if errors['exist'] == True:
        gui.msgbox(title="Attention", message=errors['message'])
        gui.update_status(status_top=title[:60], status_bottom=errors['message'], color="red")
        return
        
    # finish
    gui.update_status(status_top=title[:60], status_bottom="SUCCSSFULLY ADDED TO SET", 
                                             color="green")
